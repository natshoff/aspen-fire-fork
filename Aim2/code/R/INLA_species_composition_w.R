...
cor_da <- grid_tm %>%
 select(
  tpp_live_pr, ba_live_pr,  # species-level proportion live metrics
  ...
  tree_ht_live, tree_dia_live, # species tree height/diameter
...
da <- grid_tm %>%
 mutate(
  ...
  total_tpp = tpp_live_total + tpp_dead_total,
...
# create a wide-format table for modeling
da.w <- da %>%
 select(
  grid_index, fire_id, first_obs_date, x, y, # factor IDs and temporal effect
  log_frp_max_day, log_frp_csum_day, CBIbc_p90, # response variables
  species_gp_n, fortypnm_gp, # species information
  forest_pct, canopypct, balive, # gridcell BA, canopy percent, and forest percent
  ba_live, qmd_live, tpp_live, # species-specific structure metrics
  H_ba, H_tpp, # gridcell diversity
  erc_dv, vpd, vs, # climate/fire weather
  elev, slope, tpi, chili, # topography
  overlap, day_prop, afd_count, aspen # gridcell aggregation information
 ) %>%
 # rename the species structure metrics for cleanliness
 rename(
  ba = ba_live,
  qmd = qmd_live,
  tpp = tpp_live
 ...
 pivot_wider(
  ...
  id_cols = c(
   grid_index, fire_id, first_obs_date, x, y, # factor IDs and temporal effect
   log_frp_max_day, log_frp_csum_day, CBIbc_p90, # response variables
   fortypnm_gp, # species information
   forest_pct, canopypct, balive, # gridcell BA, canopy percent, and forest percent
   H_ba, H_tpp, # gridcell diversity
   erc_dv, vpd, vs, # climate/fire weather
   elev, slope, tpi, chili, # topography
 ...
 mutate(
  across(starts_with("ba_"), ~as.numeric(scale(.))),
  across(starts_with("tpp_"), ~as.numeric(scale(.))),
  across(starts_with("qmd_"), ~as.numeric(scale(.))),
  # scale everything else except the factor and response variables
  across(-c(grid_index, fire_id, first_obs_date, fortypnm_gp, x, y,
            log_frp_max_day, log_frp_csum_day, CBIbc_p90, aspen), ~as.numeric(scale(.)))
 )
...
mf.frp <- log_frp_csum_day ~ 1 + 
 # fortypnm_gp + # majority forest type *relative to aspen*
 fortypnm_gp * (ba_douglas_fir + ba_gambel_oak + ba_lodgepole_pine + ba_piñon_juniper +
  ba_quaking_aspen + ba_spruce_fir + ba_white_fir + ba_ponderosa_pine) + # species live BA
 # (tpp_douglas_fir + tpp_gambel_oak + tpp_lodgepole_pine + tpp_piñon_juniper + 
 #  tpp_quaking_aspen + tpp_spruce_fir + tpp_white_fir + tpp_ponderosa_pine) + # species live TPP
 # (qmd_douglas_fir + qmd_gambel_oak + qmd_lodgepole_pine + qmd_piñon_juniper + 
 #   qmd_quaking_aspen + qmd_spruce_fir + qmd_white_fir + qmd_ponderosa_pine) + # species live QMD
 H_tpp + # grid cell diversity (abundance-based)
...
X <- da.w %>% 
 select(fire_id, first_obs_date, grid_index, 
        ...
        starts_with("ba_"), starts_with("qmd_"), starts_with("tpp_"))
head(X)

# Create the INLA data stack
stack.frp <- inla.stack(
 data = list(log_frp_csum_day = da.w$log_frp_csum_day),
 A = list(A, 1),  
 tag = 'est',
 effects = list(
  c(field.idx),
