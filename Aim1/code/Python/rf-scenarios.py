...
# Plot F1
sns.boxplot(x='Feature_Set', y='Score', data=results_m[results_m['Metric'] == 'F1'], ax=axes[0])
...
# Plot MCC
sns.boxplot(x='Feature_Set', y='Score', data=results_m[results_m['Metric'] == 'MCC'], ax=axes[1])
axes[1].set_title('MCC by Feature Set')
...
plt.tight_layout()
plt.suptitle('Model Performance Metrics Across Classification Scenarios', fontsize=16)
...
# Calculate mean or median across folds for each Feature_Set
median_mcc = results.groupby('Feature_Set')['MCC'].mean()  # or median()
median_f1 = results.groupby('Feature_Set')['F1'].mean()  # or median()
