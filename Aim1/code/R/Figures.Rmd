...
## Table 4: Accuracy of the reference datasets based on test data
...
```{r}
rm(f7b,f7,blocks_,blocks_aspen)
gc()
```
...
Calculate the difference in class statistics between the different sources:

```{r}
# Calculate the grouped statistics for the SR
df <- bind_rows(class.sr_,class.wr_) %>%
  group_by(source,region) %>%
  summarize(
    total_area_km2 = sum(total_area) * 0.01,
    total_n_patch = sum(n_patch)
  ) %>%
  ungroup()
head(df, 8)

# Step 1: Filter out Sentinel-based map values
sentinel_df <- df %>% 
  filter(source == "Sentinel-based Map") %>%
  rename(sentinel_area_km2 = total_area_km2,
         sentinel_n_patch = total_n_patch) %>%
  select(region,sentinel_area_km2,sentinel_n_patch)

# Step 2: Join this with the original dataframe on region
df <- df %>%
  left_join(sentinel_df, by = "region")

# Step 3: Calculate the percentage difference
# Ensure that you're not comparing Sentinel-based map values with itself
df <- df %>%
  filter(source != "Sentinel-based Map") %>%
  mutate(area_diff = (total_area_km2 - sentinel_area_km2) / sentinel_area_km2 * 100,
         patch_diff = (total_n_patch - sentinel_n_patch) / sentinel_n_patch * 100)
head(df)

```

Calculate the difference in patch statistics between the different sources:
...
sentinel_df <- df %>% 
  filter(source == "Sentinel-based Map") %>%
  rename(sentinel_patch_size_mn = patch_size_mn) %>%
  select(region,sentinel_patch_size_mn)
...
df <- df %>%
  filter(source != "Sentinel-based Map") %>%
  mutate(percentage_difference = (patch_size_mn - sentinel_patch_size_mn) / sentinel_patch_size_mn * 100)
head(df)
```
...
```{r message=F, fig.width=7, fig.height=6.5}

# Create one map with a scale bar and north arrow

arrow <- ggspatial::north_arrow_minimal(
  line_width = 1,
  line_col = "black",
  fill = "black",
  text_col = "black",
...
p1 <- plots[[1]] + 
  ggspatial::annotation_scale(location = "bl", width_hint = 0.25, pad_x = unit(-0.01,"in"),
                              text_cex=0.6) + 
...
fs1a <- ggarrange(p1, plots[[3]], plots[[2]], plots[[6]],
